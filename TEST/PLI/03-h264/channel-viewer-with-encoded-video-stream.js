...
console.log('NIKNIK: functionalized surrogate gets and specialized h264 frames - build 0');
...
function processVideo(track, frame) {
  ...
  if (frame.data) {
    if (frame.type === 'key') {
      const results = `0x${buf2hex(frame.data)}`;
      ...
      status.innerHTML = `<p>KEY: ${results} </p><p>DELTA: 0x${(XXX_DELTA ? buf2hex(XXX_DELTA) : '')}`;
      ...
      if (!XXX_DELTA || frame.data.byteLength < XXX_DELTA.byteLegnth) {
        ...
        console.log(`DELTA: 0x${buf2hex(XXX_DELTA)}`);
      }
